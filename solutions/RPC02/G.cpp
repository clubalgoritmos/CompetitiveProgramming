...
string get_shape(vector<Point> &points) {
    vector<double> side_lengths = {dist(points[0], points[1]), dist(points[1], points[2]),
                                   dist(points[2], points[3]), dist(points[3], points[0])};
    vector<double> diag_lengths = {dist(points[0], points[2]), dist(points[1], points[3])};

    bool all_sides_equal = side_lengths[0] == side_lengths[1] &&
                           side_lengths[1] == side_lengths[2] && side_lengths[2] == side_lengths[3];
    bool all_diags_equal = diag_lengths[0] == diag_lengths[1];
    bool opposite_sides_equal =
        side_lengths[0] == side_lengths[2] && side_lengths[1] == side_lengths[3];
    bool diags_perpendicular = (points[0].x - points[2].x) * (points[1].x - points[3].x) +
